...
run_local_player(Conexao, Board, Player, NewBoard) :- 
...
run_remote_player(Conexao, Board, Player, NewBoard) :-
   recebe_jogada(Conexao, X, Y, Z),
   get_index(X, Y, Z, Index),
   set_pos(Board, Index, Player, NewBoard).
...
run_network_game(Conexao, Board, Player, RemotePlayer) :- (
   print_status(Board)
   ;(
      (Player = RemotePlayer -> run_remote_player(Conexao, Board, Player, NewBoard); run_local_player(Conexao, Board, Player, NewBoard)) -> (
         change_player(Player, NewPlayer),
         run_network_game(Conexao, NewBoard, NewPlayer, RemotePlayer)
      ); (
...
host_game(Porto) :- format('Aguardando conexão na porta ~d...~n', [Porto]), espera_adversario(Porto, Conexao), writeln('Conectado!'), network_game(Conexao, x, x).
